...
def hybrid_retrieve(query: str, embedder, collection, bm25_index, chunks: list[dict], top_k: int = 10) -> list[dict]:
    """
    Step 2: Merges semantic search (ChromaDB) and BM25 search results using Reciprocal Rank Fusion (RRF).
    """
    ...
    for idx, c in fused_map.items():
        ...
        # For compatibility with generator / logger, set main score to rrf_score
        c["score"] = c["rrf_score"]
    ...
    # Return top_k
    top_fused = fused_chunks[:top_k]
    return top_fused

def print_hybrid_comparison(query: str, semantic_chunks: list[dict], bm25_results: list[dict], hybrid_chunks: list[dict]):
    """
    Prints a beautiful comparison showing which chunks came from semantic only,
    BM25 only, or both, making the fusion benefits extremely clear!
    """
