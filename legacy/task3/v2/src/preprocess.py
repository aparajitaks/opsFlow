...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Adding Process Temp - Air Temp provides a direct signal for heat transfer rate.
    # A low temp difference means heat cannot dissipate effectively, increasing failure risk.
    processed_df['temp_diff'] = (
        processed_df['Process temperature [K]'] - processed_df['Air temperature [K]']
    )
    
    # Feature 2: Mechanical Power
    # Power is physically proportional to Torque * Angular Velocity (Rotational Speed).
    # This captures intense operational strain regimes that raw single values miss.
    processed_df['power'] = (
        processed_df['Torque [Nm]'] * processed_df['Rotational speed [rpm]']
    )
    
    # Feature 3: Tool Wear to Torque Ratio
    # Captures the interaction between tool degradation and instantaneous torque load.
    # An older, worn tool under high load is highly vulnerable to catastrophic failure.
...
def prepare_data_pipeline(df: pd.DataFrame):
    ...
    # Drop target and sub-failure columns to prevent target leakage
...
def apply_scaling(X_train: pd.DataFrame, X_test: pd.DataFrame, columns_to_scale: list):
    ...
    # Scale only specified continuous features
